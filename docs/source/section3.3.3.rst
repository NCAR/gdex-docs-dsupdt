...
(Alias: -BuildLocalFile) validates and converts remote
...
If specialist-defined validation and conversion routines are configured,
this action invokes them. When local and remote files are identical, no
action is taken. Otherwise, standard compress/uncompress and tar/untar
utilities handle one-to-one, many-to-one, and one-to-many conversions.
Local files are the final, archive-ready output.

When a local file already exists on disk, the build action for that file
is skipped. To rebuild, remove the existing local file manually.
